...
import { Assignee, Priority } from "./AddTaskModal";
...
export default function EditTaskModal({
  ...
  // sync form state whenever `task` changes
  const [title, setTitle] = useState(task.title);
  const [selectedAssignees, setSelectedAssignees] = useState<string[]>(
    task.assignments.map((a) => a.user.id)
  );
  const [dueDate, setDueDate] = useState(task.dueDate);
  const [priorityIdx, setPriorityIdx] = useState(task.priority - 1);
  const [description, setDescription] = useState(task.description);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    setTitle(task.title);
    setSelectedAssignees(task.assignments.map((a) => a.user.id));
    setDueDate(task.dueDate);
    setPriorityIdx(task.priority - 1);
    setDescription(task.description);
    setError(null);
  }, [task]);

  const handleSubmit = async () => {
    if (!title || !dueDate || !description || !selectedAssignees.length) {
                      ...
                      <Combobox.Option
                        ...
                        className={({ active, selected }) =>
                          `cursor-pointer px-3 py-2 text-sm ${
                            active ? "bg-blue-50" : ""
                          } ${selected ? "font-semibold text-[#367cf6]" : "text-gray-700"}`
                        }
                      >
                        {a.name}
                      </Combobox.Option>
